...
local function TextShadowPaint(myself, w, h)
	...
	surface.SetTextPos(myself.TextPosX or 0, myself.TextPosY or 0)
...
local cv_hide_creator = GetConVar("cl_tfa_inspect_hide_creator")
local cv_hide_manufacturer = GetConVar("cl_tfa_inspect_hide_manufacturer")
local cv_hide_capacity = GetConVar("cl_tfa_inspect_hide_capacity")
local cv_hide_ammo = GetConVar("cl_tfa_inspect_hide_ammo")
local cv_hide_contact = GetConVar("cl_tfa_inspect_hide_contact")
local cv_hide_attachments = GetConVar("cl_tfa_inspect_hide_attachments")
local cv_hide_origin = GetConVar("cl_tfa_inspect_hide_origin")
	...
	if self.StatTable.bar then
		...
		if cv_bars_exp and cv_bars_exp:GetBool() then
			...
			if self.StatTable.text then
				...
				surface.SetTextColor(TFA_INSPECTIONPANEL.SecondaryColor.r, TFA_INSPECTIONPANEL.SecondaryColor.g, TFA_INSPECTIONPANEL.SecondaryColor.b, TFA_INSPECTIONPANEL.SecondaryColor.a)
				surface.DrawText(self.StatTable.text(self.Weapon))
			end
...
function SWEP:InspectionVGUIMainInfo(contentpanel)
	if hook.Run("TFA_InspectVGUI_InfoStart", self, contentpanel) ~= false then
		...
		if self.Author and string.Trim(self.Author) ~= "" and not (cv_hide_creator and cv_hide_creator:GetBool()) then
		...
		if self.Manufacturer and string.Trim(self.Manufacturer) ~= "" and not (cv_hide_manufacturer and cv_hide_manufacturer:GetBool()) then
		...
		if clip > 0 and not (cv_hide_capacity and cv_hide_capacity:GetBool()) then
		...
		if an and an ~= "" and string.len(an) > 1 and not (cv_hide_ammo and cv_hide_ammo:GetBool()) then
		...
		if self.Purpose and string.Trim(self.Purpose) ~= "" then
			local lines = WrapTextLines(language.GetPhrase(self.Purpose), maxlinewidth, "TFA_INSPECTION_SMALL")
		...
		if self.Origin and string.Trim(self.Origin) ~= "" and not (cv_hide_origin and cv_hide_origin:GetBool()) then
			local lines = WrapTextLines(language.GetPhrase("tfa.inspect.origin"):format(self.Origin), maxlinewidth, "TFA_INSPECTION_SMALL")

			for _, line in pairs(lines) do
				local originline = infopanel:Add("DPanel")
				originline:SetName("Origin")
				originline.Text = infotextpad .. line

				originline.Think = function(myself)
					myself.TextColor = mainpanel.SecondaryColor
				end

				originline.Font = "TFA_INSPECTION_SMALL"
				originline:Dock(TOP)
				originline:SetSize(ScrW() * .5, TFA.Fonts.InspectionHeightSmall)
				originline.Paint = TextShadowPaint
			end
		end

		if self.Contact and string.Trim(self.Contact) ~= "" and not (cv_hide_contact and cv_hide_contact:GetBool()) then
			local lines = WrapTextLines(language.GetPhrase("tfa.inspect.contact"):format(self.Contact), maxlinewidth, "TFA_INSPECTION_SMALL")

			for _, line in pairs(lines) do
				local contactline = infopanel:Add("DPanel")
				contactline:SetName("Contact")
				contactline.Text = infotextpad .. line

				contactline.Think = function(myself)
					myself.TextColor = mainpanel.SecondaryColor
				end

				contactline.Font = "TFA_INSPECTION_SMALL"
				contactline:Dock(TOP)
				contactline:SetSize(ScrW() * .5, TFA.Fonts.InspectionHeightSmall)
				contactline.Paint = TextShadowPaint
			end
		end

		if self.Ability and string.Trim(self.Ability) ~= "" then
			local lines = WrapTextLines(language.GetPhrase("tfa.inspect.ability"):format(language.GetPhrase(self.Ability)), maxlinewidth, "TFA_INSPECTION_SMALL")

			for _, line in pairs(lines) do
				local abilityline = infopanel:Add("DPanel")
				abilityline:SetName("Ability")
				abilityline.Text = infotextpad .. line

				abilityline.Think = function(myself)
					myself.TextColor = mainpanel.SecondaryColor
				end

				abilityline.Font = "TFA_INSPECTION_SMALL"
				abilityline:Dock(TOP)
				abilityline:SetSize(ScrW() * .5, TFA.Fonts.InspectionHeightSmall)
				abilityline.Paint = TextShadowPaint
			end
		end
...
language.Add("tfa.inspect.stat.headshotmultiplier", "Headshot Multiplier: %sx 	[%s]")
...
SWEP.InspectionVGUIStatsTable = {
	...
	},
	{
		...
		cond = function(wep) return wep.NZHeadShotMultiplier and wep.NZHeadShotMultiplier ~= 1 end,
		text = function(wep)
			local mult = wep.NZHeadShotMultiplier
			local dmg = math.Round(wep:GetStatL("Primary.Damage") * mult)
			return language.GetPhrase("tfa.inspect.stat.headshotmultiplier"):format(mult, dmg)
		end,
		bar = function(wep) return wep.NZHeadShotMultiplier / 10 end
	},
	{
		name = "mobility",
...
function SWEP:InspectionVGUIStats(contentpanel)
	if hook.Run("TFA_InspectVGUI_StatsStart", self, contentpanel) ~= false then
		...
		statspanel:NoClipping(true)
		statspanel.Paint = function(pnl, w, h)
			local prog = self:GetInspectingProgress() or 0
			local pad = ScaleH(8)
			local bg = GetUIColorAlpha and GetUIColorAlpha("background", math.floor(math.Clamp(prog * 200, 0, 200))) or Color(21, 21, 21, 160)
			draw.RoundedBox(8, -pad, -pad, w + pad * 2, h + pad * 2, bg)
		end
		statspanel.stats = {}
...
function SWEP:GenerateInspectionDerma()
	...
	function TFA_INSPECTIONPANEL.Paint(myself, w, h)
		...
		if IsValid(wep) then
			...
			myself.SecondaryColor = ColorAlpha(GetUIColor and GetUIColor("primary") or INSPECTION_SECONDARYCOLOR, TFA_INSPECTIONPANEL.Alpha)
			...
			if not myself.SideBar then
				myself.SideBar = Material("nz_moo/nz_menu_leakmod_remake_remaster_rerun_gunga_swunga_bloodorcrip_v5_reborn.png", "noclamp smooth")
			end

			-- Draw the nzombies F1 menu texture as the full background
			local bgcol = GetUIColorAlpha and GetUIColorAlpha("f1menu", myself.Alpha) or myself.BackgroundColor
			surface.SetDrawColor(bgcol)
			surface.SetMaterial(myself.SideBar)
	...
	-- Attachments
	if not (cv_hide_attachments and cv_hide_attachments:GetBool()) then
...
function SWEP:DoInspectionDerma()
	if not IsValid(TFA_INSPECTIONPANEL) and self:GetInspectingProgress() > 0.01 then
		...
		if GetSoundPack then
			surface.PlaySound("nz_moo/effects/ui/" .. GetSoundPack() .. "/sys_open.mp3")
		end
	end
...
function SWEP:DrawFallbackHUD()
	...
	surface.SetFont("TFASleekSmall")
	surface.SetTextPos(ScrW() * .5 - w * .5, ScaleH(1017))
	surface.SetTextColor(self.TextCol.r, self.TextCol.g, self.TextCol.b, self.TextCol.a)
	surface.DrawText(fmn)
end
...
local cv_fm_notify = GetConVar("cl_tfa_hud_firemode_notify")

SWEP.FireModeNotifyFunctions = {
	[1] = function(self, msg)
...
function SWEP:FireModeCycleNotify(firemode)
	local func = self.FireModeNotifyFunctions[cv_fm_notify:GetInt()]
	if not func then return end

	local msg = language.GetPhrase("tfa.msg.firemode"):format(self:GetFireModeName(firemode))
	return func(self, msg)
end

function SWEP:AddNetworkVarWatchersCL()
	self:NetworkVarNotify("FireMode", function(entity, name, old, new)
		if old == new then return end

		local fms = self:GetStatL("FireModes")
		if #fms <= 2 or new == #fms then return end

		local ow = entity:GetOwner()
		if ow ~= GetViewEntity() and ow ~= LocalPlayer() then return end

		entity:FireModeCycleNotify(new)
	end)

	hook.Run("TFA_AddNetworkVarWatchersCL", self)
end
